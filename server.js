...
async function initDB() {
  try {
    ...
    const hadiahCount = await pool.query('SELECT COUNT(*) as count FROM hadiah');
    if (hadiahCount.rows[0].count === 0) {
    ...
    const karyCount = await pool.query('SELECT COUNT(*) as count FROM karyawan');
    if (karyCount.rows[0].count === 0) {
